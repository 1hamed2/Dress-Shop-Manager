...
export default function Imports({ imports, queryParams, success, updateMsg, deleteMsg }) {
    ...
    const handleSearch = () => {
        ...
        if (/^\d+$/.test(searchQuery)) {
            ...
            delete updatedParams.company_id;
        } else {

            updatedParams.company_id = searchQuery;
            delete updatedParams.id;
        }


        router.get(route('imports.index', updatedParams));
    };
    ...
    const [importDlt, setImportDlt] = useState(false);
    const [selectedImport, setSelectedImport] = useState(null);


    const openModal = (importss) => {
        setImportDlt(true);
        setSelectedImport(importss);
    ...
    const deleteImport = (selectedImport) => {

        // if (!window.confirm('Are you sure you want to delete this import?')) {
        //     return;
        // }
        if (!selectedImport || !selectedImport.id) {
            alert("Error: No import selected");
            return;
        }

        console.log("Deleting import:", selectedImport);

        router.delete(route('imports.destroy', selectedImport.id));
    ...
    return (
        <>
            <Admin>
                        ...
                        <Card className="h-full w-full">
                                ...
                                <div className="mb-4 flex items-center justify-between gap-8">
                                    ...
                                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                                        <Button className="flex items-center gap-3 bg-gray-900" size="sm">
                                            <Link href={route('imports.create')} className="flex items-center">

                                                <UserPlusIcon strokeWidth={2} className="h-4 w-4" /> Add import
                                            </Link>
                                        </Button>
                                    </div>
                                </div>
                                <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                                    <div className="w-full md:w-max">
                                        ...
                                        <Typography color="gray" className="mt-1 font-normal">
                                            See information about all imports
                                        </Typography>
                                    </div>
                                ...
                                <table className="mt-4 w-full min-w-max table-auto text-left">
                                        ...
                                        <tr className="bg-slate-800">
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Company</h2>
                                                    <ChevronUpDownIcon
                                                        onClick={() => sortChanged("company")}
                                                        name="company"
                                                        strokeWidth={2}
                                                        className="h-4 w-4 text-white"
                                                    />
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <ChevronUpDownIcon
                                                        onClick={() => sortChanged("total_cost")}
                                                        name="total_cost"
                                                        strokeWidth={2}
                                                        className="h-4 w-4 text-white"
                                                    />
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Updated At</h2>
                                                    <ChevronUpDownIcon strokeWidth={2} className="h-4 w-4 text-white" />
                                                </Typography>
                                            </th>
                                    ...
                                    <tbody>
                                        {imports.data.length === 0 ? (
                                            ...
                                            imports.data.map(
                                                (importss) => {
                                                    const isLast = importss === imports.length - 1;
                                                            ...
                                                            <td className={classes}>
                                                                <div>
                                                                    <Tooltip content="Delete User">
                                                                        <IconButton onClick={(e) => openModal(importss)} variant="text">
                                                                            <TrashIcon className="h-6 w-6 text-red-500" />
                                                                        </IconButton>
                                                                    </Tooltip>
                                                                    <Tooltip content="Edit User">
                                                                        <IconButton variant="text">
                                                                            <Link href={route('imports.edit', importss.id)}>
                            ...
                            <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                                <Typography variant="small" color="blue-gray" className="font-normal">
                                    Page {imports.meta.current_page} of {imports.meta.last_page}
                                </Typography>
                                <div className="flex gap-2">
                                    {imports.meta.links.map((link) => (
                                        <Link
                                            preserveScroll
                                            href={link.url || ""}
                                            key={link.label}
                ...
                <Modal show={importDlt} onClose={closeModal}>
                    <div className="p-6 pointer-events-auto">
                        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                            Are you sure you want to delete this importss?
                        </h2>
                        ...
                        <div className="mt-6 flex justify-end">
                            ...
                            <DangerButton
                                onClick={() => deleteImport(selectedImport)} className="ms-3">
                                Delete Import
                            </DangerButton>
                        </div>
                    </div>
