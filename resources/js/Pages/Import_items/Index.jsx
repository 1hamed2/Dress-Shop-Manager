...
export default function ImportItems({ importItems, queryParams, success, updateMsg, deleteMsg }) {
    ...
    const handleSearch = () => {
        ...
        if (/^\d+$/.test(searchQuery)) {
            ...
            delete updatedParams.product_id;
        } else {

            updatedParams.product_id = searchQuery;
            delete updatedParams.id;
        }


        router.get(route('import-items.index', updatedParams));
    };
    ...
    const [importItemDlt, setImportItemDlt] = useState(false);
    const [selectedImportItem, setSelectedImportItem] = useState(null);


    const openModal = (importItem) => {
        setImportItemDlt(true);
        setSelectedImportItem(importItem);

    }

    const closeModal = () => {
        setImportItemDlt(false);
        setSelectedImportItem(null);


    }

    const deleteImportItem = (selectedImportItem) => {

        // if (!window.confirm('Are you sure you want to delete this importItem?')) {
        //     return;
        // }
        if (!selectedImportItem || !selectedImportItem.id) {
            alert("Error: No importItem selected");
            return;
        }

        console.log("Deleting importItem:", selectedImportItem);

        router.delete(route('import-items.destroy', selectedImportItem.id));
    ...
    return (
        <>
            <Admin>
                        ...
                        <Card className="h-full w-full">
                                ...
                                <div className="mb-4 flex items-center justify-between gap-8">
                                    ...
                                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                                        <Button className="flex items-center gap-3 bg-gray-900" size="sm">
                                            <Link href={route('import-items.create')} className="flex items-center">

                                                <UserPlusIcon strokeWidth={2} className="h-4 w-4" /> Add import item
                                            </Link>
                                        </Button>
                                    </div>
                                </div>
                                <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                                    <div className="w-full md:w-max">
                                        ...
                                        <Typography color="gray" className="mt-1 font-normal">
                                            See information about all import items
                                        </Typography>
                                    </div>
                                ...
                                <table className="mt-4 w-full min-w-max table-auto text-left">
                                        ...
                                        <tr className="bg-slate-800">
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Company name</h2>
                                                    <ChevronUpDownIcon
                                                        onClick={() => sortChanged("import_id")}
                                                        name="import_id"
                                                        strokeWidth={2}
                                                        className="h-4 w-4 text-white"
                                                    />
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Product Name</h2>
                                                    <ChevronUpDownIcon
                                                        onClick={() => sortChanged("product_id")}
                                                        name="product_id"
                                                        strokeWidth={2}
                                                        className="h-4 w-4 text-white"
                                                    />
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Quantity</h2>
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Cost Per Unit</h2>
                                                    <ChevronUpDownIcon strokeWidth={2} className="h-4 w-4 text-white" />
                                                </Typography>
                                            </th>
                                            <th
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Total Cost</h2>
                                                    <ChevronUpDownIcon strokeWidth={2} className="h-4 w-4 text-white" />
                                                </Typography>
                                            </th>
                                    ...
                                    <tbody>
                                        {importItems.data.length === 0 ? (
                                            ...
                                            importItems.data.map(
                                                (importItem) => {
                                                    const isLast = importItem === importItems.length - 1;
                                                        ...
                                                        <tr key={importItem.id}>
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    <div className="flex flex-col">
                                                                        <Typography
                                                                            ...
                                                                            className="font-normal"
                                                                        >
                                                                            {importItem.import?.company?.name}
                                                            ...
                                                            <td className={classes}>
                                                                ...
                                                                >
                                                                    ...
                                                                    {formatPKR(importItem.cost_per_unit)}
                                                            ...
                                                            <td className={classes}>
                                                                ...
                                                                >
                                                                    ...
                                                                    {formatPKR(importItem.total_cost)}
                                                                    <span className="ml-1 px-1 py-0.5 bg-gray-200 text-gray-500 text-[10px] rounded-full">PKR</span>
                                                                </Typography>
                                                            </td>
                                                            <td className={classes}>
                                                                <div>
                                                                    <Tooltip content="Delete User">
                                                                        <IconButton onClick={(e) => openModal(importItem)} variant="text">
                                                                            <TrashIcon className="h-6 w-6 text-red-500" />
                                                                        </IconButton>
                                                                    </Tooltip>
                                                                    <Tooltip content="Edit User">
                                                                        <IconButton variant="text">
                                                                            <Link href={route('import-items.edit', importItem.id)}>
                            ...
                            <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                                <Typography variant="small" color="blue-gray" className="font-normal">
                                    Page {importItems.meta.current_page} of {importItems.meta.last_page}
                                </Typography>
                                <div className="flex gap-2">
                                    {importItems.meta.links.map((link) => (
                                        <Link
                                            preserveScroll
                                            href={link.url || ""}
                                            key={link.label}
                ...
                <Modal show={importItemDlt} onClose={closeModal}>
                    <div className="p-6 pointer-events-auto">
                        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                            Are you sure you want to delete this importItem?
                        </h2>
                        ...
                        <div className="mt-6 flex justify-end">
                            ...
                            <DangerButton
                                onClick={() => deleteImportItem(selectedImportItem)} className="ms-3">
                                Delete Import item
                            </DangerButton>
                        </div>
                    </div>
