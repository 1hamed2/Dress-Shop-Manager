...
export default function Incomes({ incomes, queryParams, success, updateMsg, deleteMsg }) {
    ...
    const handleSearch = () => {
        ...
        if (searchQuery.trim().length > 0) {
            updatedParams.month = searchQuery.trim();
        }

        router.get(route('incomes.index', updatedParams));
    };
    ...
    const [incomeDlt, setIncomeDlt] = useState(false);
    ...
    const deleteIncome = (selectedIncome) => {

        // if (!window.confirm('Are you sure you want to delete this income?')) {
        //     return;
        // }
        if (!selectedIncome || !selectedIncome.id) {
            alert("Error: No income selected");
            return;
        }

        console.log("Deleting income:", selectedIncome);
    ...
    return (
        <>
            <Admin>
                        ...
                        <Card className="h-full w-full">
                                ...
                                <div className="mb-4 flex items-center justify-between gap-8">
                                    ...
                                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                                        <Button className="flex items-center gap-3 bg-gray-900" size="sm">
                                            <Link href={route('incomes.create')} className="flex items-center">

                                                <UserPlusIcon strokeWidth={2} className="h-4 w-4" /> Add income
                                            </Link>
                                        </Button>
                                    </div>
                                </div>
                                <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                                    <div className="w-full md:w-max">
                                        ...
                                        <Typography color="gray" className="mt-1 font-normal">
                                            See information about all incomes
                                        </Typography>
                                    </div>
                                ...
                                <table className="mt-4 w-full min-w-max table-auto text-left">
                                        ...
                                        <tr className="bg-slate-800">
                                            ...
                                            >
                                                <Typography
                                                    ...
                                                    className="flex items-center justify-between gap-2 font-bold leading-none"
                                                >
                                                    <h2 className="bg-slate-800  text-white p-3 rounded">Amount</h2>
                                    ...
                                    <tbody>
                                        {incomes.data.length === 0 ? (
                                            ...
                                            incomes.data.map(
                                                (income) => {
                                                    const isLast = income === incomes.length - 1;
                                                            ...
                                                            <td className={classes}>
                                                                <div>
                                                                    <Tooltip content="Delete User">
                                                                        <IconButton onClick={(e) => openModal(income)} variant="text">
                            ...
                            <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                                <Typography variant="small" color="blue-gray" className="font-normal">
                                    Page {incomes.meta.current_page} of {incomes.meta.last_page}
                                </Typography>
                                <div className="flex gap-2">
                                    {incomes.meta.links.map((link) => (
                                        <Link
                                            preserveScroll
                                            href={link.url || ""}
                                            key={link.label}
                ...
                <Modal show={incomeDlt} onClose={closeModal}>
                    <div className="p-6 pointer-events-auto">
                        ...
                        <div className="mt-6 flex justify-end">
                            ...
                            <DangerButton
                                onClick={() => deleteIncome(selectedIncome)} className="ms-3">
                                Delete Income
                            </DangerButton>
                        </div>
                    </div>
